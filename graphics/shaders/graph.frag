...
float ray(vec3 ro, vec3 rd, float tmin, float tmax, float eps) {
    ...
    int nmax = 10;
    int i=0;
    float prev_v = intersection(ro,rd,tmin), v;
    
    if (abs(prev_v)<eps) return prev_v;
    
    for (; i<=imax; i++) {
        v = intersection(ro,rd,t);
    ...
    for (; i<=imax; i++) {
        float v_new = intersection(ro,rd,t);
...
void main() {
    float anglexz = mouse.x*PI;
    float anglev = mouse.y*PI;
...
//    fragtexpos *= 2.0;
    ...
    fragtexpos.x *= aspect;
//    fragtexpos -= vec2(aspect, 1.0);

    vec3 rd = -normalize(vec3(1.0,power*fragtexpos.y,power*fragtexpos.x));
    ...
    float tmax = 100.0;
    float t = ray(ro,rd,0.0,tmax,0.001);
    ...
    outColor += axes(axes_center, fragtexpos, anglexz, -anglev);
    ...
    outColor += light(vec4(1,1,1,1),vec4(1,1,1,1),n,lightd,va);
    outColor += light(vec4(1,1,1,1),vec4(1,1,1,1),n,normalize(lightpt-pos),va);
